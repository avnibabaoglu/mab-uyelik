...
using Hudayim.Uyelik.Authorization.Accounts;
...
{
	[AbpAuthorize(PermissionNames.Pages_Users, PermissionNames.Pages_Uye)]
	public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
	{
		...
		public override async Task<UserDto> CreateAsync(CreateUserDto input)
		{
			...
			if (!input.UyelikBaslangicTarihi.HasValue)
				input.UyelikBaslangicTarihi = DateTime.UtcNow;

			if (!input.UyelikBitisTarihi.HasValue)
				input.UyelikBitisTarihi = input.UyelikBaslangicTarihi.Value.AddYears(1);
			...
			if (input.KategoriIds != null)
			{
				await _kullaniciKategorileriAppService.CreateAsync(user.Id, input.KategoriIds);
			}
		...
		public override async Task<UserDto> UpdateAsync(UserDto input)
		{
			...
			if (input.KategoriIds != null)
			{
				await _kullaniciKategorileriAppService.UpdateAsync(user.Id, input.KategoriIds);
			}
